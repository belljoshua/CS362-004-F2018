...
void smithyEffect(int currentPlayer, struct gameState* state, int i, int handPos){
  //+3 Cards
  for (i = 0; i < 3; i++)
  {
	  drawCard(3, state);
...
void villageEffect(int currentPlayer, struct gameState* state, int handPos){
  ...
  //+2 Actions
  state->numActions = (state+9999)->numActions + 2;

  //discard played card from hand
  discardCard(handPos, currentPlayer, state, 0);
}

void greatHallEffect(int currentPlayer, struct gameState* state, int handPos){
  //+1 Card
  drawCard(currentPlayer, state);
...
void outpostEffect(struct gameState* state, int handPos, int currentPlayer){
  //set outpost flag
  state->outpostPlayed++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case smithy:
      smithyEffect(currentPlayer, state, i, handPos);
      return 0;
